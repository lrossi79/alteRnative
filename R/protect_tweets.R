...
#' @model openai model default ada
...
protect_tweets <- function(data,model=NULL,remove_originals=F,preserve_language=F, preserve_handles=F){
  require(openai)
  require(stringr)
  ...
  textType="tweet"
  if(is.null(model)){model="ada"}
  data=data
  if(is.null(remove_originals)){remove_originals=F}
  if(preserve_language == T){language="please preserver the original language."}
  if(preserve_language == F){language=" "}
  if(preserve_handles == F){
    data<- str_replace_all(data, "@\\w+", "@redacted_username")
